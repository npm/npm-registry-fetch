...
// Checks for scope- or package-specific registry for the package, or given scope
function pickRegistry (pkg, opts = {}) {
  const spec = npa(pkg)
  let registry

  // Try name from spec first
  if (spec.name) {
    registry = opts[`${spec.name}:registry`]
  }

  // Try scope from spec next
  if (!registry && spec.scope) {
    registry = opts[`${spec.scope}:registry`]
  }

  // Try scope from opts last, optionally prefixed with "@"
  if (!registry && opts.scope) {
    registry = opts[`${opts.scope.replace(/^@?/, '@')}:registry`]
  }

  // Fall back to opts, then defaults
  if (!registry) {
